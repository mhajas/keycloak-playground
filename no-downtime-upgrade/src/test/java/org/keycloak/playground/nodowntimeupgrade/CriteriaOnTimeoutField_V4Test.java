...
import static org.keycloak.playground.nodowntimeupgrade.VersionUtil_V3.getTimeoutFromV4;
 ...
 * @author hmlnarik
 */
public class CriteriaOnTimeoutField_V4Test extends AbstractNoDowntimeUpgradeTest {
    ...
    public void testFieldEquals() {
        Integer key = INITIAL_INDEX_V4 + 4;

        List<ObjectModel_V4> objects;
        final int timeout = getTimeoutFromV4(key);

        ModelCriteriaBuilder cb = storageV4.getCriteriaBuilder()
          .compare(TIMEOUT1, Operator.EQ, timeout);

        objects = storageV4.read(cb).collect(Collectors.toList());
        assertThat(objects, hasSize(
          + (int) v4indices().filter(i -> VersionUtil_V3.getTimeoutFromV4(i) == timeout).count() // !!! No values from V3
        ));
        for (ObjectModel_V4 m : objects) {
            VersionUtil_V4.V4_UTIL.assertValid(m);
            assertThat(m.getTimeout1(), is(timeout));
        }
        ...
        cb = storageV4.getCriteriaBuilder()
          .compare(TIMEOUT1, Operator.EQ, timeout)
          .compare(TIMEOUT1, Operator.EQ, getTimeout(key + 1));
    ...
    public void testFieldNotEquals() {
        ...
        ModelCriteriaBuilder cb = storageV4.getCriteriaBuilder()
          .compare(TIMEOUT1, Operator.NE, timeout);

        objects = storageV4.read(cb).sorted(Comparator.comparing(HasId::getId)).collect(Collectors.toList());
        assertThat(objects, hasSize(
            INITIAL_COUNT_V1
          + INITIAL_COUNT_V3
        ...
        for (ObjectModel_V4 m : objects) {
            VersionUtil_V4.V4_UTIL.assertValid(m);
            assertThat(m.getTimeout1(), not(equalTo(timeout)));
        }
        assertThat(objects.stream().map(HasId::getId).collect(Collectors.toSet()), not(contains(key)));

        cb = storageV4.getCriteriaBuilder()
          .compare(TIMEOUT1, Operator.NE, timeout)
          .compare(TIMEOUT1, Operator.NE, timeout1);

        assertThat(storageV4.read(cb).collect(Collectors.toList()), hasSize(
            INITIAL_COUNT_V1
